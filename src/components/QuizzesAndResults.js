...
const QuizzesAndResults = () => {
  ...
  useEffect(() => {
    const fetchPdfs = async () => {
      try {
        const res = await axios.get('https://meep-back.onrender.com/api/documents');
        setPdfs(res.data);
        setLoading(false);
      } catch (err) {
  ...
  const handleDownload = async (pdfId, pdfFileName) => {
    try {
      const response = await axios.get(`https://meep-back.onrender.com/api/documents/${pdfId}/download`, {
