...
const ManageQuiz = () => {
  ...
  const handleEditQuestion = (questionId) => {
    // Navigate to the edit page for the selected question
    Router.push(`/edit-question/${questionId}`);
  };

  const handleArchiveQuestion = async (questionId) => {
    try {
      // Send a request to archive the question
      await axios.put(`http://localhost:8080/api/quiz/${questionId}`);
      // Update the local state to remove the archived question
