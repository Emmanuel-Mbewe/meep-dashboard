...
const SubtopicCard = ({ subject, topic, subtopic, description, imageUrl, onEdit, onDelete }) => {
  const [isHovered, setIsHovered] = React.useState(false);
  const router = useRouter();

  const handleCardClick = () => {
    router.push(`/subjects/${subject}/${topic}/${subtopic}`);
  };
  ...
  return (
    ...
    >
      <img src={imageUrl} alt={subtopic} style={imageStyle} />
      <div>
        <h2>{subtopic}</h2>
        <p>{description}</p>
      </div>
